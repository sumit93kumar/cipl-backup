...
$columns = array(
	...
	array( 'db' => "case when um.class='A' then '1' else '0' end ",     'dt' => 8, 'field' => 'class','formatter' => function( $d, $row ) {
																	return '<button class="btn btn-default" type="button">'.$d.'</button>';
																},'as'=>'class' )
);
...
$joinQuery = "FROM `user` AS `u` JOIN `user_details` AS `ud` ON (`ud`.`user_id` = `u`.`id`) join user_id_master AS um on (um.userid = u.id)";
