         ...
         <div class="container">
            ...
            <div class="row">
               <div class="col-lg-6 offset-lg-3 text-center noPadding">
                  ...
                  <h2 class="sec_title">
                     <?php 
						$query = $conn->prepare("Select * from category where id = ".$_GET['id']);
						$query->execute();
						if($query->rowCount() > 0){
							$row  = $query->fetchAll(PDO::FETCH_ASSOC);
							//print_r($row);
							foreach($row as $sr){
								echo $sr['category_name'];
							}
						}
					 ?>
                  </h2>
			...
			<div class="row">
					...
					<?php
						$id = $_GET['id']; 
						$query = $conn->prepare("Select subcategory.*, (select min(name) from files where status=1 and gallery.images = files.title)  as images from subcategory left join gallery on gallery.subcat_id = subcategory.id  where subcategory.cat_id =".$id." and subcategory.status =1 group by subcategory.subcategory_name;");
						$query->execute();
						$fd = '';
						if($query->rowCount() > 0){
							$row = $query->fetchAll(PDO::FETCH_ASSOC);
							...
							foreach($row as $dr){
								...
								if($dr["images"] == ''){
									$fd .= '<div class="col-md-4">
													<a href="gallery-cat.php?id='.$dr["subcategory_name"].'">
													<div class="singleblog bg-shadws">
														<img src="images/no-img.jpg" alt="">
														<div class="sc-date"><p class="date">'.$dr["subcategory_name"].'</p></div>
													</div>
													</a>
												</div>';
								}
								...
								$fd .= '<div class="col-md-4">
													<a href="gallery-cat.php?id='.$dr["subcategory_name"].'">
													<div class="singleblog bg-shadws">
														<img src="admin/php/files/'.$dr["images"].'" alt="">
														<div class="sc-date"><p class="date">'.$dr["subcategory_name"].'</p></div>
													</div>
													</a>
												</div>';
